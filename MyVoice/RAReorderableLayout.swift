...
open class RAReorderableLayout: UICollectionViewFlowLayout, UIGestureRecognizerDelegate {
    ...
    fileprivate func moveItemIfNeeded() {
        ...
        collectionView!.performBatchUpdates({
            ...
            print("Deleted item at ")
            print(atIndexPath)
            print("inserted item to  ")
            print(toIndexPath)
    ...
    fileprivate func cancelDrag(_ toIndexPath: IndexPath!) {
        ...
        cellFakeView!.pushBackView {
            ...
            print("Cancel Drag")
            //FIXME: Need to update the order of the Files n the directory, maybe fetch themp initialy 3ader but then sort them by rank which will consitute the order, for now neglect the draggin process
